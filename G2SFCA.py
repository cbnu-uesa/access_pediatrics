...
def ABFCA(demand_data, supply_data, cost_data, threshold):
    ...
    data['w_supply'] = data['supply_val'] * data['fij']
    w_supply = data.groupby('demand_id', as_index = False)\
        .agg(sum_w_supply = ('w_supply', 'sum'))
    data = data.merge(w_supply, how = 'left', on = 'demand_id')
    data['qij'] = data['w_supply'] / data['sum_w_supply']

    # 1단계: 공급지별 수요(w_demand)를 고려한 Rj 산정 
    #data['w_demand'] = data['demand_val'] * data['qij']
    data['w_demand'] = data['demand_val'] * data['qij'] * data['fij']
    w_demand = data.groupby('supply_id', as_index = False)\
        .agg(sum_w_demand = ('w_demand', 'sum'))
    ...
    data['w_rj'] = data['rj'] * data['qij'] * data['fij']
    access = data.groupby('demand_id', as_index = False)\
        .agg(accessibility = ('w_rj', 'sum'))
...
cost_data = pd.read_csv('data/new_network_data.csv')
demand_data = pd.read_csv('data/new_demand_data.csv')
supply_data = pd.read_csv('data/new_supply_data.csv')
threshold = 30


# Execute the modified 2SFCA with ABFC adjustment
result_g2sfca, data_g2sfca = Generalized_2SFCA(demand_data, supply_data, cost_data, threshold)

result_abfca, data_abfca = ABFCA(demand_data, supply_data, cost_data, threshold)

result_g2sfca.to_csv(f'output/g2sfca_{threshold}.csv', index=False, encoding = 'utf-8-sig')
result_abfca.to_csv(f'output/abfca_{threshold}.csv', index=False, encoding = 'utf-8-sig')
